...
class GraspInferenceNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("object_depth_margin", 0.10)  # meters: obj = height_hm < max - margin
        ...
        self.object_depth_margin = float(self.get_parameter("object_depth_margin").value)
    ...
    def _on_heightmaps(self, color_msg: Image, height_msg: Image, mask_msg: Image):
        ...
        # ----- Маска: валидные данные, без верхних 75 строк -----
        valid = np.isfinite(height_hm) & (height_hm != 0)
        valid[:75, :] = False
        ...
        try:
            ...
            center_camera.header.stamp = color_msg.header.stamp
            center_camera.header.frame_id = color_msg.header.frame_id
            center_camera.pose.position.x = cx_cam
        ...
        heatmap[~keep] = 0
        heatmap = cv2.circle(heatmap, (center_col, center_row), 8, (0, 255, 0), 2)  # green = object center
        heatmap = cv2.circle(heatmap, (col, row), 6, (0, 0, 255), 2)                 # red = grasp point
