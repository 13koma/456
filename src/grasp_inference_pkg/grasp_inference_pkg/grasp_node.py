...
class HeightmapNode(Node):
    def __init__(self) -> None:
        ...
        try:
            ...
            self.get_logger().warning(
                f"YOLO segmentation disabled: failed to load '{seg_model_path}': {e}"
            )
    ...
    def _segment_union_mask(self, rgb_u8: np.ndarray, cache_key: str = "pcd"):
        ...
        if self.seg is None:
            # Временный режим без сегментации: пропускаем весь кадр
    ...
    def _srv_reset_accumulator(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        self._reset_accumulator()
        self.get_logger().info("Accumulator reset via service call")
        response.success = True
        response.message = "Accumulator reset"
        return response
