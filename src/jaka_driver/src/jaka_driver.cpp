...
void joint_position_callback(const rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr& joint_position_pub)
{
    ...
    // Получить полный статус робота (включая velocities)
    int ret = robot.get_robot_status(&robotstatus);
    if (ret != 0) {
        // Fallback - получить хотя бы позиции
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        // ДОБАВЛЕНО: Скорость из RobotStatus
        if (ret == 0) {
            joint_position.velocity.push_back(
                robotstatus.robot_monitor_data.jointMonitorData[i].instVel
            );
        } else {
            joint_position.velocity.push_back(0.0);  // Fallback если ошибка
        }
    }
    
    joint_position.header.stamp = rclcpp::Clock().now();
    joint_position.header.frame_id = "base_link";
...
int main(int argc, char *argv[])
{
    ...
    robot.set_status_data_update_time_interval(8);
