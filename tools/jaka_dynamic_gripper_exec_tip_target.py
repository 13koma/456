...
class JakaDynamicGripperExec(Node):
    def __init__(self) -> None:
        ...
        # Transit / one-action preview
        self.declare_parameter("use_transit_waypoints", False)
        self.declare_parameter("transit_clearance_m", 0.10)

        # Target interpretation
        self.declare_parameter("target_is_gripper_tip", False)
        self.declare_parameter("gripper_tip_offset_x_m", 0.15)
        self.declare_parameter("gripper_tip_offset_y_m", 0.0)
        self.declare_parameter("gripper_tip_offset_z_m", 0.0)
    ...
    @staticmethod
    def rpy_to_rotation_matrix(rpy: tuple[float, float, float]) -> list[list[float]]:
        roll, pitch, yaw = rpy
    ...
    @staticmethod
    def rotate_vector(rot: list[list[float]], vec: tuple[float, float, float]) -> tuple[float, float, float]:
        return (
    ...
    def target_pose_for_motion(self, pose: Pose3, label: str) -> Pose3:
        if not bool(self.get_parameter("target_is_gripper_tip").value):
            return pose

        cmd_rpy = self.ik_rpy()
        rot = self.rpy_to_rotation_matrix(cmd_rpy)
        tip_offset_tool = (
            float(self.get_parameter("gripper_tip_offset_x_m").value),
            float(self.get_parameter("gripper_tip_offset_y_m").value),
            float(self.get_parameter("gripper_tip_offset_z_m").value),
        )
        tip_offset_base = self.rotate_vector(rot, tip_offset_tool)
        tool_goal = Pose3(
            pose.x - tip_offset_base[0],
            pose.y - tip_offset_base[1],
            pose.z - tip_offset_base[2],
        )
        self.get_logger().info(
            f"{label}: interpreting target as gripper_tip | "
            f"tip_offset_tool=({tip_offset_tool[0]:.3f}, {tip_offset_tool[1]:.3f}, {tip_offset_tool[2]:.3f}) "
            f"tip_offset_base=({tip_offset_base[0]:.3f}, {tip_offset_base[1]:.3f}, {tip_offset_base[2]:.3f}) "
            f"tool_goal=({tool_goal.x:.3f}, {tool_goal.y:.3f}, {tool_goal.z:.3f}) "
            f"ik_rpy={self.rpy_deg_str(cmd_rpy)}"
        )
        return tool_goal
    ...
    def move_via_transit(self, goal: Pose3, stage_name: str) -> bool:
        if self.tool is None:
            self.get_logger().error("tool pose is not available for transit")
            return False
        cur = self.current_pose3()
        clearance = float(self.get_parameter("transit_clearance_m").value)
        transit_z = max(cur.z, goal.z) + clearance
        up_goal = Pose3(cur.x, cur.y, transit_z)
        xy_goal = Pose3(goal.x, goal.y, transit_z)

        self.get_logger().info(
            f"{stage_name}: using transit waypoints | "
            f"up=({up_goal.x:.3f}, {up_goal.y:.3f}, {up_goal.z:.3f}) "
            f"xy=({xy_goal.x:.3f}, {xy_goal.y:.3f}, {xy_goal.z:.3f}) "
            f"final=({goal.x:.3f}, {goal.y:.3f}, {goal.z:.3f})"
        )

        if not self.move_until_reached(up_goal, f"{stage_name} / TRANSIT up"):
            return False
        if not self.move_until_reached(xy_goal, f"{stage_name} / TRANSIT xy"):
            return False
        if not self.move_until_reached(goal, f"{stage_name} / TRANSIT down"):
            return False
        return True
    ...
    def run(self) -> bool:
        ...
        grasp_goal = self.target_pose_for_motion(grasp_tip_goal, "STAGE 2 fine approach")
        self.get_logger().info(
        ...
        self.get_logger().info(
            f"STAGE 2 desired tip grasp_above=({grasp_tip_goal.x:.3f}, {grasp_tip_goal.y:.3f}, {grasp_tip_goal.z:.3f})"
        )

        if use_transit:
            stage2_ok = self.move_via_transit(grasp_goal, "STAGE 2 fine approach")
        else:
            stage2_ok = self.move_until_reached(grasp_goal, "STAGE 2 fine approach")
        if not stage2_ok:
            self.maybe_return_to_start(start_pose, "return_to_start_on_failure")
            return False
